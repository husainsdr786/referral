...
class ScraperService
{
    ...
    public function scrapMyntra($url){
        
        $client  = new Client(HttpClient::create(['timeout' => 60]));
        $crawler = $client->request('GET', $url);
        
        $title = $crawler->filter('.index-sizeFitDesc h2')->each(function ($node) {
            return $node->text();
        });

        //Get the link to view a specific job details
        $links = $crawler->filter('.index-sizeFitDesc .index-sizeFitDescTitle .index-product-description-title')->each(function($node){
            $href  = 'https://www.myntra.com' . $node->attr('href');
            //$title = $node->attr('title');
            $text  = $node->text();
            return compact('href', 'title', 'text');
        });
